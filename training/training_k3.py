...
class training():
    ...
    def apply_model(self, images, labels):
        ...
        test_output_p = outputs[0, :3, :, :].cpu().detach().numpy().transpose(1, 2,
                                                                              0)  # test_output_p.shape: (512, 1024, 3)
        test_output_g0 = outputs[0, 3:13, :, :].cpu().detach().numpy().transpose(1, 2, 0)
        test_output_g1 = outputs[0, 13:17, :, :].cpu().detach().numpy().transpose(1, 2, 0)
        test_output_g2 = outputs[0, 17:26, :, :].cpu().detach().numpy().transpose(1, 2, 0)
        ...
        mask_g1 = (p == 5) | (p == 6) | (p == 7)
        mask_g2 = (p == 11) | (p == 12) | (p == 13) | (p == 14) | (p == 15) | (p == 16) | (p == 17) | (p == 18)
        p[mask_g0 == True] = 0
        p[mask_g1 == True] = 1
        p[mask_g2 == True] = 2
        ...
        group1 = [5, 6, 7]
        group2 = [11, 12, 13, 14, 15, 16, 17, 18]
        ...
        # ==== group 2 ====
        # q21 person, trainid = 11
        q21_vis = torch.where(target_vis == 11, torch_ones * 1, ignore_only).long()
        q21_occ = torch.where(target_occ == 11, torch_ones * 1, ignore_only).long()
        # q22 rider, trainid = 12
        q22_vis = torch.where(target_vis == 12, torch_ones * 2, ignore_only).long()
        q22_occ = torch.where(target_occ == 12, torch_ones * 2, ignore_only).long()
        # q23 car, trainid = 13
        q23_vis = torch.where(target_vis == 13, torch_ones * 3, ignore_only).long()
        q23_occ = torch.where(target_occ == 13, torch_ones * 3, ignore_only).long()
        # q24 truck, trainid = 14
        q24_vis = torch.where(target_vis == 14, torch_ones * 4, ignore_only).long()
        q24_occ = torch.where(target_occ == 14, torch_ones * 4, ignore_only).long()
        # q25 bus, trainid = 15
        q25_vis = torch.where(target_vis == 15, torch_ones * 5, ignore_only).long()
        q25_occ = torch.where(target_occ == 15, torch_ones * 5, ignore_only).long()
        # q26 train, trainid = 16
        q26_vis = torch.where(target_vis == 16, torch_ones * 6, ignore_only).long()
        q26_occ = torch.where(target_occ == 16, torch_ones * 6, ignore_only).long()
        # q27 motorcycle, trainid = 17
        q27_vis = torch.where(target_vis == 17, torch_ones * 7, ignore_only).long()
        q27_occ = torch.where(target_occ == 17, torch_ones * 7, ignore_only).long()
        # q28 bicycle, trainid = 18
        q28_vis = torch.where(target_vis == 18, torch_ones * 8, ignore_only).long()
        q28_occ = torch.where(target_occ == 18, torch_ones * 8, ignore_only).long()
        ...
        q00_target_vis = p[:, 0, :, :].clone().reshape(self.batch_size, 1, self.height, self.width)
        q00_target_occ = p[:, 1, :, :].clone().reshape(self.batch_size, 1, self.height, self.width)
        ...
        q10_target_vis = p[:, 0, :, :].clone().reshape(self.batch_size, 1, self.height, self.width)
        q10_target_occ = p[:, 1, :, :].clone().reshape(self.batch_size, 1, self.height, self.width)
        ...
        q20_target_vis = p[:, 0, :, :].clone().reshape(self.batch_size, 1, self.height, self.width)
        q20_target_occ = p[:, 1, :, :].clone().reshape(self.batch_size, 1, self.height, self.width)
        ...
        ########################## losses p's ##########################
        loss_p = CrossEntropyLoss2d(self.weight[:3])(outputs[:, :3, :, :], target_p)
        # print("loss_p:", loss_p)
        ########################## losses q00, q10, q20 ##########################
        loss_q00 = CrossEntropyLoss2d(self.weight[3:13])(outputs[:, 3:13, :, :], q00)
        loss_q10 = CrossEntropyLoss2d(self.weight[13:17])(outputs[:, 13:17, :, :], q10)
        loss_q20 = CrossEntropyLoss2d(self.weight[17:26])(outputs[:, 17:26, :, :], q20)
        # print("loss_q00:", loss_q00)
        # print("loss_q10:", loss_q10)
        # print("loss_q20:", loss_q20)
        ########################## losses for visible ##########################
        # g0: q01-q09
        loss_q01_vis = CrossEntropyLoss2d(self.weight[3:13])(outputs[:, 3:13, :, :], q01_vis)
        loss_q02_vis = CrossEntropyLoss2d(self.weight[3:13])(outputs[:, 3:13, :, :], q02_vis)
        loss_q03_vis = CrossEntropyLoss2d(self.weight[3:13])(outputs[:, 3:13, :, :], q03_vis)
        loss_q04_vis = CrossEntropyLoss2d(self.weight[3:13])(outputs[:, 3:13, :, :], q04_vis)
        loss_q05_vis = CrossEntropyLoss2d(self.weight[3:13])(outputs[:, 3:13, :, :], q05_vis)
        loss_q06_vis = CrossEntropyLoss2d(self.weight[3:13])(outputs[:, 3:13, :, :], q06_vis)
        loss_q07_vis = CrossEntropyLoss2d(self.weight[3:13])(outputs[:, 3:13, :, :], q07_vis)
        loss_q08_vis = CrossEntropyLoss2d(self.weight[3:13])(outputs[:, 3:13, :, :], q08_vis)
        loss_q09_vis = CrossEntropyLoss2d(self.weight[3:13])(outputs[:, 3:13, :, :], q09_vis)

        # g1: q11-q13
        loss_q11_vis = CrossEntropyLoss2d(self.weight[13:17])(outputs[:, 13:17, :, :], q11_vis)
        loss_q12_vis = CrossEntropyLoss2d(self.weight[13:17])(outputs[:, 13:17, :, :], q12_vis)
        loss_q13_vis = CrossEntropyLoss2d(self.weight[13:17])(outputs[:, 13:17, :, :], q13_vis)

        # g2: q21-q28
        loss_q21_vis = CrossEntropyLoss2d(self.weight[17:26])(outputs[:, 17:26, :, :], q21_vis)
        loss_q22_vis = CrossEntropyLoss2d(self.weight[17:26])(outputs[:, 17:26, :, :], q22_vis)
        loss_q23_vis = CrossEntropyLoss2d(self.weight[17:26])(outputs[:, 17:26, :, :], q23_vis)
        loss_q24_vis = CrossEntropyLoss2d(self.weight[17:26])(outputs[:, 17:26, :, :], q24_vis)
        loss_q25_vis = CrossEntropyLoss2d(self.weight[17:26])(outputs[:, 17:26, :, :], q25_vis)
        loss_q26_vis = CrossEntropyLoss2d(self.weight[17:26])(outputs[:, 17:26, :, :], q26_vis)
        loss_q27_vis = CrossEntropyLoss2d(self.weight[17:26])(outputs[:, 17:26, :, :], q27_vis)
        loss_q28_vis = CrossEntropyLoss2d(self.weight[17:26])(outputs[:, 17:26, :, :], q28_vis)
        ...
        loss_vis_g2 = [loss_q21_vis, loss_q22_vis, loss_q23_vis, loss_q24_vis, loss_q25_vis, loss_q26_vis,
                       loss_q27_vis, loss_q28_vis]

        ########################## losses for occluded ##########################
        # g0: q01-q09
        loss_q01_occ = CrossEntropyLoss2d(self.weight[3:13])(outputs[:, 3:13, :, :], q01_occ)
        loss_q02_occ = CrossEntropyLoss2d(self.weight[3:13])(outputs[:, 3:13, :, :], q02_occ)
        loss_q03_occ = CrossEntropyLoss2d(self.weight[3:13])(outputs[:, 3:13, :, :], q03_occ)
        loss_q04_occ = CrossEntropyLoss2d(self.weight[3:13])(outputs[:, 3:13, :, :], q04_occ)
        loss_q05_occ = CrossEntropyLoss2d(self.weight[3:13])(outputs[:, 3:13, :, :], q05_occ)
        loss_q06_occ = CrossEntropyLoss2d(self.weight[3:13])(outputs[:, 3:13, :, :], q06_occ)
        loss_q07_occ = CrossEntropyLoss2d(self.weight[3:13])(outputs[:, 3:13, :, :], q07_occ)
        loss_q08_occ = CrossEntropyLoss2d(self.weight[3:13])(outputs[:, 3:13, :, :], q08_occ)
        loss_q09_occ = CrossEntropyLoss2d(self.weight[3:13])(outputs[:, 3:13, :, :], q09_occ)

        # g1: q11-q13
        loss_q11_occ = CrossEntropyLoss2d(self.weight[13:17])(outputs[:, 13:17, :, :], q11_occ)
        loss_q12_occ = CrossEntropyLoss2d(self.weight[13:17])(outputs[:, 13:17, :, :], q12_occ)
        loss_q13_occ = CrossEntropyLoss2d(self.weight[13:17])(outputs[:, 13:17, :, :], q13_occ)

        # g2: q21-q28
        loss_q21_occ = CrossEntropyLoss2d(self.weight[17:26])(outputs[:, 17:26, :, :], q21_occ)
        loss_q22_occ = CrossEntropyLoss2d(self.weight[17:26])(outputs[:, 17:26, :, :], q22_occ)
        loss_q23_occ = CrossEntropyLoss2d(self.weight[17:26])(outputs[:, 17:26, :, :], q23_occ)
        loss_q24_occ = CrossEntropyLoss2d(self.weight[17:26])(outputs[:, 17:26, :, :], q24_occ)
        loss_q25_occ = CrossEntropyLoss2d(self.weight[17:26])(outputs[:, 17:26, :, :], q25_occ)
        loss_q26_occ = CrossEntropyLoss2d(self.weight[17:26])(outputs[:, 17:26, :, :], q26_occ)
        loss_q27_occ = CrossEntropyLoss2d(self.weight[17:26])(outputs[:, 17:26, :, :], q27_occ)
        loss_q28_occ = CrossEntropyLoss2d(self.weight[17:26])(outputs[:, 17:26, :, :], q28_occ)
        ...
        loss = loss_p + loss_g0 + loss_g1 + loss_g2
